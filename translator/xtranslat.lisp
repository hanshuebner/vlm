...
(clos:defclass ivory-instruction ()
    ((pc :initarg :pc :accessor ivory-instruction-pc :initform -1)
     (order :initarg :order :accessor ivory-instruction-orderD,#TD1PsT[Begin using 006 escapes](1 0 (NIL 0) (NIL :BOLD NIL) "CPTFONTCB") :initform -10)
...
(clos:defmethod linkup-function ((decfcn decoded-function))
  (clos:with-slots (instructions) decfcn
	...
	((null instn)
	 ;; Finally sort1 0the instructions into execution order
	 (setq instructions (sort instructions #'< :key 'ivory-instruction-order))
	 1(do ((instns instructions (cdr instns0)1))
...
(clos:defmethod add-instruction ((nuinst ivory-instruction) (istream translation-state))
  (clos:with-slots (target pendinglabel) istream
	...
	1(when 0pendinglabel
	  1(setq label (append label (shiftf 0pendinglabel1 0nil1))))
0	(setq order icount)
...
0	      1do (0assemble-asm-form1 (shiftf *function-epilogue* nil) 0tstate1)))
0      )
    ...
    (decode-operand-specifier (relto offset popp) operand
      (cond ((eq relto :immediate)
	    ...
	    ((lisp:and popp (not (eq dest 'isp)))
	     ...
	     ;; adjust offset
	     (values (map-ivory-register relto) (* 8 (+ offset 1))1 t0))
	    (:otherwise
	     ;;; the :sp :lp :fp cases (not sp-pop). justload the value into the
	     ;;; target.
	     (values (map-ivory-register relto) (* 8 offset)1 popp0))))))
...
(defun allocate-1specific-0registers (reglist1 0istream)
...
(defun release-1specific-0registers (reglist1 0istream)
  (clos:with-slots (freeregs) istream
    (dolist (reg reglist)
...
(defmacro with-1specific0-registers ((&rest registers) &body body)
  (if (null registers) 
      `(progn ,@body)
      `(1progn
0	 (allocate-1specific-0registers 1',0registers1 istream0)
	 (unwind-protect
	     (progn ,@body)
	   (release-1specific-0registers 1',0registers1 istream0)))))
